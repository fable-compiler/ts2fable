...
module TypeScriptSpecialRules =
    ...
    let enabled = 
        // Note: at this point we do not have a fake context
        // -> Exception: `System.Exception: Cannot retrieve 'Fake.Core.Target.ArgResults' as we have no fake context`
        // Target.getArguments () 
        // |> Option.defaultValue [||]
...
let init args =
  ...
  Target.create "Clean" <| fun _ ->
      // `dotnet clean` wants to clean `build/fake/` too -- which fails because it is in use (this build script)
      [ cliDir; appDir; testDir; testCompileDir]
      |> List.iter (execDotNet "clean")
      Shell.cleanDirs [
          cliBuildDir
          testBuildDir
          distDir
      ...
      if (isAppveyor) then
          ...
          if  repoName = "fable-compiler/ts2fable" && repoBranch = "master" then
              ...
              let handle addtionalBehavior =
                  ...
                  Repository.clone "./" "-b dev git@github.com:fable-compiler/ts2fable-exports.git" repositoryDir
              ...
              if String.isNullOrEmpty prHeadRepoName then
                  handle
                      ( fun () ->
                          Branches.checkoutBranch repositoryDir "master"
                          Merge.merge repositoryDir "-X theirs" "dev"
                          Staging.stageAll repositoryDir
                          Branches.push repositoryDir
                          )
              else
  ...
  Target.create "WebApp.Publish" <| fun _ ->
      if (isAppveyor) then
          ...
          if repoName = "fable-compiler/ts2fable" && repoBranch = "master" && String.isNullOrEmpty prHeadRepoName then
              ...
              Repository.cloneSingleBranch "" url "gh-pages" "gh-pages"

              Repository.fullClean "gh-pages"
              Shell.copyRecursive "web-app/output" "gh-pages" true |> printfn "%A"
              Staging.stageAll "gh-pages"
